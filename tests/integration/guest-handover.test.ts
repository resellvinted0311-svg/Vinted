 ...
 *
 * Favoris, panier et commandes payées sans compte appartiennent au JETON de
 * session boutique. Ce jeton est renouvelé à chaque ouverture de session — sur
 * un poste partagé, la personne suivante ne doit pas hériter de ce que la
 * précédente avait mis de côté. Tout ce qui n'a pas basculé AVANT ce
 * renouvellement est perdu définitivement.
 *
 * Un cas était déjà perdu : `mergeGuestCart` existait, testée, et n'était
 * appelée par aucun code. Le panier d'un visiteur ne survivait pas à sa
 * connexion.
 */
...
describe('renouvellement du jeton', () => {
  ...
  it('pose un jeton neuf même sans rien à reprendre', async () => {
    ...
    expect(report).toEqual({ favorites: 0, cartLines: 0, orders: 0 })
